...
public class AutomatedTester {
    ...
    private String[] productName = { "Eggs 12pk", "Corn Chips", "Apples 2lb",
            "Cookies", "Bread", "Soda 12pk", "Watermelon", "Bananas 1lb",
            "Salad Mix", "CerealA", "Pasta", "Marinara Sauce", "Salsa",
    ...
    private String[] currentPrice = { "4.50", "3.25", "5.00", "1.75", "2.75",
            "4.25", "2.00", "0.95", "4.75", ".35", "1.50", "5.95", "4.85",
            "1.99", "7.85", "14.50", "5.15", "9.00", "6.75", "299" };
    ...
    public void testAddProduct() {
        ...
        Result result = Store.instance().addProduct(Request.instance());
        assert result.getResultCode() == Result.PRODUCT_EXISTS : result
                .getResultCode();
        Request.instance().setProductId("P21");
        result = Store.instance().addProduct(Request.instance());
        assert result.getResultCode() == Result.NAME_IN_USE : result
                .getResultCode();
    }
    ...
    /**
     * Automated test method to test the processShipment functionality. This
     * test will process order #O1. It first shows that the initial inventory of
     * the product received is 10 units. The test then runs processShipment()
     * and shows that the OPERATION_COMPLETE code is received. It then shows
     * that the Product's stock on hand has been increased to 20 units. (Reorder
     * level for this product is 5, so a new order will always be 10 units.)
     * Finally, the test runs with order #O200, which does not exist, to show
     * that the NO_ORDER_FOUND code is received.
     * 
     * @author Marc Wedo
     */
